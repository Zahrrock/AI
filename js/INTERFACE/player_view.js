function getBorderingLocations(){ // Retourne à l'AI tous les objets visibles (ceux qui sont juste à côté)
    ...
    for (let i = 0; i < 5; i++) { // Pour tous les emplacements autour du joueur et l'emplacement du joueur lui-même
        ...
        const element_on_location = ENV.Locations[location_to_check.x + ENV.player.position_record.x[0]][location_to_check.y + ENV.player.position_record.y[0]]; // Récupérer ce qu'il y a vraiment à cet emplacement
        if(element_on_location === undefined){ // Condition de sécurité. Si element_on_location est indéfini dans ENV.Locations, il y a un problème quelque part
            console.error("element_on_location is undefined. coordonnées : ", location_to_check.x + ENV.player.position_record.x[0], " ", location_to_check.y + ENV.player.position_record.y[0]);
        }
        if(Array.isArray(element_on_location)){ // condition vérifiée si l'élément est un tableau (si il y a plusieurs objets sur l'emplacement)
            ...
            console.error("L'objet aux coordonnées : ", location_to_check.x + ENV.player.position_record.x[0], " ", location_to_check.y + ENV.player.position_record.y[0], " ne rentre dans aucune condition");
        }
    }
    if(bordering_locations.length !== 5){ // Condition de sécurité
        console.error("bordering_locations.length !== 5");
    }
